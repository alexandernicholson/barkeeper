#!/usr/bin/env bash
# etcd behavioral benchmark — captures reference output for comparison with barkeeper
set -euo pipefail

OUTDIR="$(dirname "$0")/results/etcd"
mkdir -p "$OUTDIR"
DATA_DIR=$(mktemp -d)
LISTEN_PORT=23791
LISTEN_PEER_PORT=23801
...
echo "=== Starting etcd on port $LISTEN_PORT ==="
etcd \
    --name test-etcd \
    --data-dir "$DATA_DIR" \
    --listen-client-urls "http://127.0.0.1:$LISTEN_PORT" \
    --advertise-client-urls "http://127.0.0.1:$LISTEN_PORT" \
    --listen-peer-urls "http://127.0.0.1:$LISTEN_PEER_PORT" \
    --initial-advertise-peer-urls "http://127.0.0.1:$LISTEN_PEER_PORT" \
    --initial-cluster "test-etcd=http://127.0.0.1:$LISTEN_PEER_PORT" \
    --log-level error \
    &>/dev/null &
ETCD_PID=$!
sleep 2
...
HTTP="http://127.0.0.1:$LISTEN_PORT"
...
echo ""
echo "=== etcd benchmark complete. Results in $OUTDIR ==="
